  ...
  const handleSendMessage = () => {
    ...
    setTimeout(() => {
      ...
      setMessages(prev => [...prev, { 
        role: 'bot', 
        text: "Entendi! Estou processando sua solicitação. Como sou uma demonstração, posso te ajudar com dúvidas gerais ou te encaminhar para um especialista no WhatsApp.",
      }]);
    }, 1500);
  };
          ...
          >
              ...
              {messages.length === 0 ? (
                <>
                  ...
                  <div className="flex flex-col mb-5">
                    <button onClick={() => setInputValue("Qual plano faz sentido para minha operação?")} className="flex items-start gap-4 py-3 border-b border-zinc-100/80 hover:bg-zinc-50/50 transition-colors text-left group">
                      ...
                      <span className="text-sm text-zinc-700 font-medium leading-snug">Qual plano faz sentido para minha operação?</span>
                    </button>
                    <button onClick={() => setInputValue("Como escalar vendas via WhatsApp?")} className="flex items-start gap-4 py-3 border-b border-zinc-100/80 hover:bg-zinc-50/50 transition-colors text-left group">
                      <svg className="w-4 h-4 text-zinc-400 mt-0.5 group-hover:text-[#B597FF] transition-colors shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M7 17l9.2-9.2M17 17V7H7"/></svg>
                      <span className="text-sm text-zinc-700 font-medium leading-snug">Como escalar vendas via WhatsApp?</span>
                    </button>
                    <button onClick={openWhatsApp} className="flex items-center gap-4 py-3 px-4 bg-green-50/50 rounded-xl mt-4 hover:bg-green-100 transition-colors text-left group border border-green-100 shadow-sm">
                      ...
                      <span className="text-sm text-green-800 font-bold leading-snug">Falar com a equipe via WhatsApp</span>
                    </button>
                  </div>
                </>
              ) : (
            ...
            <div className="px-4 pb-4 bg-white">
               ...
               <p className="text-center text-[10px] text-zinc-400 mt-3 font-medium">
                 {messages.length > 0 ? "A Lia está em modo demonstração." : "A Lia pode cometer erros. Verifique as respostas."}
               </p>
            </div>
          </motion.div>
        )}
