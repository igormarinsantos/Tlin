...
export function LiaPopup() {
  ...
  const [messages, setMessages] = useState<{ role: 'user' | 'bot', text: string, type?: 'handoff' }[]>([]);
  const [inputValue, setInputValue] = useState("");
  const [isTyping, setIsTyping] = useState(false);
  const [status, setStatus] = useState(t.liaPopup.online);
  const [isHovered, setIsHovered] = useState(false);
  const [canShow, setCanShow] = useState(false);
  const scrollRef = useRef<HTMLDivElement>(null);

  const placeholder = t.liaPopup.placeholder;

  // Only show button after Hero is likely done
  useEffect(() => {
    const timer = setTimeout(() => setCanShow(true), 2500);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        if (data.handoff) {
           setTimeout(() => {
              setMessages(prev => [...prev, { role: 'bot', text: "", type: 'handoff' }]);
           }, 1000);
        }
      }, 800);
    } catch (error) {
      setIsTyping(false);
      setStatus(t.liaPopup.online);
      setMessages(prev => [...prev, { role: 'bot', text: "Desculpe, tive um probleminha. Pode tentar de novo?" }]);
    }
  };

  const openWhatsApp = () => {
    window.open("https://wa.me/5511916248604", "_blank");
  };
  ...
  const WhatsAppHandoff = () => (
    <motion.div 
      ...
      className="w-full bg-green-50 border border-green-100 rounded-2xl p-4 mt-2 flex flex-col items-center text-center gap-3"
    >
      ...
      <div className="flex flex-col">
        <span className="text-sm font-black text-zinc-900">{t.liaPopup.handoffTitle}</span>
        <span className="text-[11px] font-medium text-zinc-500 mt-0.5">{t.liaPopup.handoffSubtitle}</span>
      </div>
      <button 
        onClick={openWhatsApp}
        className="w-full py-3 bg-green-500 hover:bg-green-600 text-white rounded-xl font-bold text-xs transition-all shadow-md shadow-green-100"
      >
        {t.liaPopup.handoffBtn}
      </button>
    </motion.div>
  );
          ...
          >
            ...
            <div className="flex items-center justify-between p-4 bg-white border-b border-zinc-100 shrink-0">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-zinc-100 border border-zinc-200 overflow-hidden shrink-0 relative">
                  <Image 
                    src="/LIA PERFIL.png" 
                    alt="Lia" 
                    fill
                ...
                <div className="flex flex-col gap-4 py-4">
                  {messages.map((msg, i) => (
                    <div key={i} className={`flex items-start gap-2 ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                      {msg.role === 'bot' && msg.type !== 'handoff' && (
                         <div className="w-8 h-8 rounded-full bg-zinc-100 border border-zinc-200 overflow-hidden shrink-0 mt-1 relative">
                           <Image 
                             src="/LIA PERFIL.png" 
                             alt="Lia" 
                             fill
                  ...
                  {isTyping && (
                    <div className="flex items-start gap-2">
                       <div className="w-8 h-8 rounded-full bg-zinc-100 border border-zinc-200 overflow-hidden shrink-0 mt-1 shadow-sm relative">
                         <Image 
                           src="/LIA PERFIL.png" 
                           alt="Lia" 
                           fill
