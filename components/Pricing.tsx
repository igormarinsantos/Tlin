...
function MarketComparison() {
  ...
  const HeaderIcon = ({ column }: { column: "agent" | "crm" | "human" }) => {
    ...
    return <span className="flex h-7 w-7 items-center justify-center rounded-lg bg-zinc-100 text-[#0c0d0d]"><Icon className="h-3.5 w-3.5" strokeWidth={1.9} /></span>;
  };
  ...
  return <section className="mt-24 border-t border-zinc-100 pt-24 md:mt-32 md:pt-32">
    ...
    <div className="mt-12 hidden overflow-hidden rounded-[2rem] border border-zinc-200 bg-white md:block"><div className="grid grid-cols-[1.9fr_repeat(4,1fr)] border-b border-zinc-100 bg-[#FCFCFD]"><div className="flex items-end px-7 pb-5 text-[11px] font-bold uppercase tracking-wide text-zinc-400">O que evita perder oportunidades</div>{columns.map((column) => <div key={column.key} className={`flex min-h-[76px] items-center justify-center px-3 py-4 text-center ${column.key === "tlin" ? "bg-[#F3FDFF]" : ""}`}>{column.key === "tlin" ? <Image src="/Logo%20Horizontal.svg" alt="Tlin" width={70} height={26} className="h-6 w-auto" /> : <div className="flex flex-col items-center gap-1.5"><HeaderIcon column={column.key} /><p className="text-sm font-bold text-[#0c0d0d]">{column.label}</p></div>}</div>)}</div>{COMPARISON_ROWS.map((row, index) => <div key={row.label} className={`grid grid-cols-[1.9fr_repeat(4,1fr)] ${index < COMPARISON_ROWS.length - 1 ? "border-b border-zinc-100" : ""}`}><div className="flex items-center px-7 py-4 text-sm font-semibold leading-snug text-[#0c0d0d]">{row.label}</div>{columns.map((column) => <div key={column.key} className={`flex items-center justify-center ${column.key === "tlin" ? "bg-[#38E3FF]/[0.06]" : ""}`}><StatusMark enabled={row[column.key]} /></div>)}</div>)}</div>
  </section>;
}
